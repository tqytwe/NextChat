...
分辨率、比例和时长，并将真实提交结果交给既有 `/api/v1/mobile/video/jobs` 合同。
客户端不得把聊天会话、聊天模型或虚构能力替换为视频会话。

服务端状态：ID `45` 是否能实际创建视频任务取决于其现有能力、价格和调度账号
配置。若用户需要其模型可执行，最小后端修复应仅补齐该组模型的现有视频能力和
价格映射；本轮没有修改该配置或后端代码。
